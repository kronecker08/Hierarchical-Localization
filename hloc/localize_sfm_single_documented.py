...
def pose_from_cluster(qname, qinfo, db_ids, db_images, points3D,
    ...
    kpq = feature_file[qname]['keypoints'].__array__() ## keypoints of the query #image
    ...
    for i, db_id in enumerate(db_ids):
        ...
        points3D_ids = db_images[db_id].point3D_ids   ### what are we getting here                                                                                                                                      ## this is the id of the 3d #point  
        pair = names_to_pair(qname, db_name)
        matches = match_file[pair]['matches0'].__array__()  ## this is what I have 
        # and what format is that you have  // Do I care? 
...
def main(reference_sfm, queries, retrieval, features, matches, results,
    ...
    assert retrieval.exists(), retrieval   #list of query and the best matches 
           
    assert features.exists(), features     # two different features one for model                                              #and another   one for the query image
    assert matches.exists(), matches
       #again two matches files one for db and another query
    
    
    queries = parse_image_lists_with_intrinsics(queries)  ## images and their intrisics ## which in our case will be only an image and its intrincs
    ## Will need to send as a list of set
    # ("name", camera params)
    retrieval_dict = parse_retrieval(retrieval)
    ## returns a default dictionary
    # Will need to make a default dict example is in pA
    ...
    for qname, qinfo in tqdm(queries):
        ...
        ## can directly send db_name/ no need for dirty work ;)
        db_ids = []
        for n in db_names:
        ...
        ## can even directly send db ids/ no need to do dirty work ;)
